<?php
// api.php - JSON API for Appointment Scheduler (clients, services, appointments)
...
try {
    ...
    if ($action === 'create_voter' && $method === 'POST') {
        ...
        if ($stmt->execute()) json_response(['success' => true, 'voter_id' => $GLOBALS['mysqli']->insert_id]);
    ...
    // --- REGISTER / CAST VOTE / RESULTS ---
    if ($action === 'register_voter' && $method === 'POST') {
        // alias for create_voter
        $_GET['action'] = 'create_voter';
    }
    ...
    if ($action === 'cast_vote' && $method === 'POST') {
        ...
        if ($GLOBALS['mysqli']->errno === 1062) json_response(['error' => 'voter has already voted in this election'], 409);
